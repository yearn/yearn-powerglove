...
interface VaultActivityChartProps {
...
function getTimeframeStartTimestamp(data: VaultActivityData, timeframe: string): number | null {
  if (timeframe === 'all') {
    return null
  }
...
function filterActivitySeries(
  ...
  const startTimestamp = getTimeframeStartTimestamp(data, timeframe)
  const isInTimeframe = (timestamp: number) => startTimestamp === null || timestamp >= startTimestamp
  const chartSeries = buildVaultActivityChartSeries(data)
      ...
      <div className="grid gap-1.5">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="tabular-nums">
            {formatPercent(point.unlockPercent, { decimals: 3, fallback: 'No data' })}
          </span>
        </div>
        <div className="flex items-center justify-between gap-3">
          <span>Rate/day</span>
...
export const VaultActivityChart: React.FC<VaultActivityChartProps> = React.memo(
  ({ activityData, timeframe, isLoading, error }) => {
    const isMobile = useIsMobile()
    const filteredActivity = useMemo(
      () => (activityData ? filterActivitySeries(activityData, timeframe) : { series: [], harvestEvents: [] }),
      [activityData, timeframe]
    )
    const isV2LockedProfit = activityData?.currentUnlock?.profitUnlockMode === 'v2_locked_profit'
    const unlockLabel = isV2LockedProfit ? 'Locked profit %' : 'Unlock %'
    ...
    const hasUnlockData = filteredActivity.series.some(
      (point) =>
    ...
    if (filteredActivity.series.length === 0) {
      return (
        <div className="flex h-full items-center justify-center border border-dashed border-border px-4 text-center text-sm text-muted-foreground">
          No harvest/unlock activity in this timeframe.
        </div>
      )
    }
    ...
    return (
      <div className="relative h-full">
        ...
        <ChartContainer
          config={{
            unlockPercent: { label: unlockLabel, color: '#111111' },
        ...
        >
          ...
          >
            ...
            <YAxis
              ...
              tickLine={{ stroke: 'hsl(var(--muted-foreground))' }}
            />
            <Tooltip content={<ActivityTooltip unlockLabel={unlockLabel} />} />
            ...
            <Line
              ...
              stroke="var(--color-unlockPercent)"
              strokeWidth={2}
              dot={false}
...
VaultActivityChart.displayName = 'VaultActivityChart'

export default VaultActivityChart
