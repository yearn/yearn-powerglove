...
export function ComparisonPanel() {
  ...
  const { sorted: sortCategories } = catSort
  ...
  const sortedCats = useMemo(
    () =>
      data
        ? sortCategories(data.byCategory, {
  ...
  return (
    <>
      <p className="text-dim" style={{ marginBottom: '1rem', fontSize: '0.82rem' }}>
        Comparing our internally-calculated TVL against DefiLlama's reported figures for yearn-finance and
        yearn-curating protocols.
      </p>
      ...
      {data.gapComponents.length > 0 && (
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {data.gapComponents.map((g) => (
              <div
                key={`${g.label}-${g.amount}`}
                style={{
                  display: 'flex',
      ...
      {data.notes.length > 0 && (
        <div className="card" style={{ padding: '0.75rem 1rem', marginBottom: '1rem' }}>
          ...
          <ul style={{ margin: 0, paddingLeft: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            {data.notes.map((note) => (
              <li key={note} className="text-dim" style={{ fontSize: '0.82rem', lineHeight: 1.5 }}>
                {note}
              </li>
            ))}
          </ul>
